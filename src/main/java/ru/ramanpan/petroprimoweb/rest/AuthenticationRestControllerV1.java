...
//            response.put("token", token);
            ...
            return new ActualUserDTO(user.getId(),user.getCreated(),user.getNickname(),user.getFullname(),user.getEmail(),user.getRole().name(),user.getDescription(),user.getPicture(),token);
        } catch (AuthenticationException e) {
            return new ActualUserDTO();
        }
    }
    @PostMapping("/register")
    public ResponseEntity.BodyBuilder register(@RequestBody RegistrationRequestDTO request) {
        User user = new User();
